...
var isCloneable = (value) => isObject(value) && !import_node_util.types.isProxy(value) && !isFn(value) && ![RegExp, Date, Promise, Map, Set, WeakMap, WeakSet].some((c) => value instanceof c);
...
var load = (_0) => __async(null, [_0], function* ({ resolved: id }) {
...
var processResource = (ctx) => {
  ...
  return pipe(
    ...
    (c) => prepare2(c, __spreadProps(__spreadValues({}, hctx), { vmap: vmap2 }))
  );
};
...
var extendArray = ({ result, sources, prefix, rules }) => {
  ...
  if (rule === "ignore" /* IGNORE */) return result;
  if (rule === "merge" /* MERGE */) {
...
var extendObject = ({ result, sources, prefix, rules, index }) => {
  for (const source of sources) {
    for (const key of getProps(source)) {
      ...
      if (rule === "ignore" /* IGNORE */) continue;
...
var _clone = ({
  ...
  vmap: vmap2 = _vmap,
  prefix = ""
}) => seed ? (memo.has(value) || memo.set(value, seed), getProps(value).reduce((m, k) => {
...
// src/main/ts/populate.ts
var populate = (_0, ..._1) => __async(null, [_0, ..._1], function* (config, opts = {}) {
